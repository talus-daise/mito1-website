const names = [
    // 2A
    "相田和", "相田悠乃介", "安藤陸", "飯塚悠仁", "石田晴大", "伊藤希", "岩島綾子", "岩田樹弥",
    "薄井圭護", "江幡遥人", "小曽納璃佳", "菊池顕広", "木許咲希", "久保田涼斗", "小林史佳",
    "小林拓海", "坂井敬伍", "篠﨑咲良", "清水玲菜", "志村拓真", "菅田悠暉", "鈴木将弘", "田口翔清",
    "田村孝太", "塚田旭", "ドカン", "堂田有紗", "戸間替咲希", "野中美遥", "芳賀かえで", "人見桜子",
    "二川ひなた", "古川陽菜", "星音羽", "松井結愛", "山口陽花", "山下真歩", "山本朱璃", "山本悠二朗",
    "渡辺太造",
    
    // 2B
    "青山ひかり", "有馬暁都", "飯村和香", "石川陽菜", "石川眞帆", "石原遼門", "市川聡真",
    "稲田光孝", "井村百花", "岩淵由真", "岩元祥一郎", "小國丈太朗", "河野蒼士", "木津碧咲", "黒澤遼",
    "小泉裕貴", "小林蒼依", "西城奈那", "佐川慶一郎", "佐々木和義", "清水和奏", "白土咲恵", "杉本桔花",
    "武田証", "飛田愼也", "直井大和", "中里ゆり奈", "野尻裕貴", "板東すずな", "廣瀬太一", "福田翼",
    "船尾帆夏", "古川陽菜", "松嶋隼士", "松本悠菜", "森啓惺", "森山心愛", "八木優歌", "安川愛菜", "山田優希",

    // 1A
    "相原文広", "赤塚壮介", "麻植雅文", "伊藤龍太", "岩井怜央", "江淵昊輝", "大川晴", "大原京花", "小貫雄平",
    "小野稜弥", "金澤真帆", "熊倉琉眞", "栗田駿", "小鶴載衡", "後藤笑真", "小橋川明", "小松﨑結子", "紺野真友",
    "佐々木理帆", "沢田葉", "白濵理子", "杉井知佳子", "鈴木美友", "住川漣", "高橋拓杜", "田中音羽", "田山楓季",
    "柘植悠那", "永井沙佳", "中西りこ", "中村葵琴", "根本樹", "根本佑香", "野村和花", "早谷直季", "藤芳直生",
    "峯岸隼吾", "宮本陽菜", "和田稜正", "渡部柚",

    // 1B
    "芦分悠夏", "臼井真之介", "江坂颯良斗", "枝川聡汰", "江成暁佑", "太田結仁", "大貫慧里衣", "小幡香菜子",
    "風間敬太", "梶美咲", "小橋楠々", "佐藤美菜子", "芝間つむぎ", "嶋田こころ", "嶋根沙希", "清水龍太",
    "鈴木朋花", "住川遼", "曽我あかね", "高柳祐梨奈", "谷越百華", "田村香晴", "德宿はるか", "戸髙創司",
    "富永光貴", "内藤咲良", "七里春樹", "西中千晃", "西野莉佳子", "橋本健", "濱野敬弘", "平野陽大",
    "平間琴葉", "深谷澪一朗", "増山幸之介", "峯岸拓吾", "森一眞", "森田彩未", "森野志保", "山崎昇龍",

    // 3A
    "浅見太一", "芦田喜一", "有田隼飛", "一家優太", "海野心絆", "大河葉奈歌", "太田和希", "柿原佑真", "木村仁",
    "黒川大智", "黒澤誠", "小林賢佑", "小松﨑宗汰", "佐々木紳伍", "佐藤航大", "塩谷華奈", "杉本美風",
    "鈴木朝陽", "鈴木茉莉香", "髙橋希", "巽美陽", "蓼沼歩", "飛田真奈", "中川愛菜", "南指原礼人", "西墻芽咲",
    "萩原仁子", "羽田琉美香", "百武佳穂", "平石暖馬", "平間太惺", "布施未杜", "古橋宙富", "本田瑛一朗",
    "前澤実日子", "町井智恵実", "マリーソフィア麗花", "丸茂愛佳", "門間恭汰", "山内尚香",

    // 3B
    "安部徳晃", "石﨑遼", "稲川陽太", "上野陽葵", "江﨑園乃", "大内彩愛", "大西昴", "大沼奏太", "小瀬蒼",
    "金谷一輝", "狐塚結月", "木村菜々子", "熊野御堂樹里", "黒澤礼", "今野心宇", "境菜々香", "佐藤明日香",
    "塩幡篤杜", "清水千尋", "清水優", "新藤礼菜", "鈴木彩夏", "鈴木波琉", "鈴木隆太", "諏訪美桜利", "添田紗愛",
    "堤望結", "中川航輝", "中根貴大", "二階堂華", "沼田凜太朗", "野手悠理", "濱田歩乃実", "平根吏人",
    "藤芳莉子", "本田創甫", "持丸涼穏", "山口史華", "山田歩茉"
];

const verificationKey = "verification";

function checkVerification() {
    const verified = localStorage.getItem(verificationKey) === "true" ? true : false;

    if (verified) return;

    const alertBackground = document.createElement("div");
    alertBackground.id = "custom-alert-background";
    Object.assign(alertBackground.style, {
        position: "fixed", zIndex: "1001", background: "#000", top: "0", left: "0",
        width: "100%", height: "100%", opacity: "0", transition: "opacity 0.5s ease-in-out"
    });

    const alertContainer = document.createElement("div");
    alertContainer.id = "custom-alert";
    Object.assign(alertContainer.style, {
        position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)",
        padding: "20px", background: "#abc3ff", border: "2px solid #00134c", zIndex: "1002",
        boxShadow: "0px 4px 10px rgba(0,0,0,0.2)", borderRadius: "8px", textAlign: "center", opacity: "0",
        transition: "opacity 0.5s ease-in-out"
    });

    const messageHead = document.createElement("h3")
    messageHead.innerText = "認証"

    const messageElem = document.createElement("p");
    messageElem.innerText = "プライバシーの面から名前の入力をお願いします\n一度入力した場合は、もう一度入力してもらい、連絡していただけると助かります。\nなお、違う端末で開いた場合は再度入力になることがあります\n※名前は間に空白を開けず漢字で入力してください。";
    messageElem.style.marginBottom = "15px";

    const nameBox = document.createElement("input");
    nameBox.id = "question-name";
    nameBox.type = "text";

    const closeButton = document.createElement("button");
    closeButton.innerText = "OK";
    Object.assign(closeButton.style, {
        padding: "5px 15px", cursor: "pointer", border: "1px solid #00134c",
        background: "#3498db", color: "#00134c", borderRadius: "5px",
        boxShadow: "0px 0px 5px 1px rgba(0, 0, 0, 0.6)"
    });
    closeButton.onclick = () => {
        alertContainer.style.opacity = "0";
        const clientName = nameBox.value;
        const isValid = names.includes(clientName);
        localStorage.removeItem(verificationKey);
        localStorage.setItem(verificationKey, isValid);
        setTimeout(() => alertBackground.remove(), 500);
        let clientUrl = window.location.pathname;
        window.location.pathname = isValid ? clientUrl : "caution.html";
    };

    alertContainer.append(messageHead, messageElem, nameBox, closeButton);
    alertBackground.appendChild(alertContainer);
    document.body.appendChild(alertBackground);

    setTimeout(() => {
        alertBackground.style.opacity = "1";
        alertContainer.style.opacity = "1";
    }, 10);
}

// Call the checkVerification function
checkVerification();
